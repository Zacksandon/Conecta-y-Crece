...
-- 1. AÑADIR 'progreso' A PROYECTOS (SI NO EXISTE)
ALTER TABLE proyectos 
ADD COLUMN IF NOT EXISTS progreso INT DEFAULT 0;

-- 2. AÑADIR 'foto_url' A USUARIOS (SI NO EXISTE)
ALTER TABLE usuarios
ADD COLUMN IF NOT EXISTS foto_url VARCHAR(255) NULL DEFAULT NULL;

-- 3. CORREGIR COLUMNA 'tipo' EN REACCIONES
-- Si existe 'tipo_reaccion' → renombrar
ALTER TABLE reacciones 
CHANGE COLUMN IF EXISTS tipo_reaccion tipo 
ENUM('me_gusta', 'me_encanta', 'me_interesa', 'apoyo') NOT NULL DEFAULT 'me_gusta';

-- Si NO existe 'tipo_reaccion' pero SÍ 'tipo' → asegurar tipo correcto
ALTER TABLE reacciones 
MODIFY COLUMN IF EXISTS tipo 
ENUM('me_gusta', 'me_encanta', 'me_interesa', 'apoyo') NOT NULL DEFAULT 'me_gusta';

-- 4. ASEGURAR QUE LA TABLA 'reacciones' TENGA LA COLUMNA CORRECTA
-- Si no existe 'tipo', créala
ALTER TABLE reacciones 
ADD COLUMN IF NOT EXISTS tipo 
ENUM('me_gusta', 'me_encanta', 'me_interesa', 'apoyo') NOT NULL DEFAULT 'me_gusta';

-- 5. ELIMINAR DUPLICADOS (por si acaso)
DELETE t1 FROM reacciones t1
INNER JOIN reacciones t2 
WHERE t1.id > t2.id 
  AND t1.proyecto_id = t2.proyecto_id 
  AND t1.usuario_id = t2.usuario_id 
  AND t1.tipo = t2.tipo;

-- 6. ÍNDICE ÚNICO PARA EVITAR DUPLICADOS FUTUROS
ALTER TABLE reacciones 
ADD UNIQUE KEY IF NOT EXISTS unique_reaccion (proyecto_id, usuario_id, tipo);
...
-- CORREGIR REACCIONES: tipo_reaccion → tipo
ALTER TABLE reacciones 
CHANGE COLUMN IF EXISTS tipo_reaccion tipo 
ENUM('me_gusta', 'me_encanta', 'me_interesa', 'apoyo') NOT NULL DEFAULT 'me_gusta';
